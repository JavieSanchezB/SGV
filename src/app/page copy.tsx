...
export default function ConsultarPunto() {
  const [id_omt, setCodPunto] = useState('');
  ...
  async function fetchPunto() {
    if (!id_omt) return;
    try {
      const res = await fetch(`/api/establecimiento?id_omt=${id_omt}`);
      const responseData = await res.json();
      if (responseData.data) {
        setData(responseData.data);
        toast.success("estableciemiento Encontrado", {
          duration: 1000,
...
/>
      ...
      {data && (
        <div className="data-container">
          <p><strong>IDPDV:</strong> {data.id_omt || 'No disponible'}</p>
